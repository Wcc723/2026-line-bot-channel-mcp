---
description: 設定 LINE Messaging API channel access token、channel secret、tunnel mode 與 webhook port。透過 _configure_set / _configure_show MCP tools 寫入 ~/.claude/channels/line/.env。
---
...
- `tunnel-mode <quick|named|external>` → 呼叫 `_configure_set` 帶 `tunnel_mode`。
- `port <number>` → 呼叫 `_configure_set` 帶 `webhook_port`。
- `public-url <url>` → 呼叫 `_configure_set` 帶 `public_url`（named/external 模式必填）。
- `wizard` 或無法解析的輸入 → 進入互動模式：依序問 token、secret、tunnel mode，每輸入一筆即呼叫 `_configure_set` 一次。
